...
- **Verified master list** — an audit ledger: 60 research entries (three passes of 20, each verified line-by-line before being added), plus flagged irregularities, limitations and the complete source directory
...
│   └── research-ledger.json   ← 60 dated research entries (20 per pass) and source links
├── scripts/
...
- **Data cutoff: September 24, 2026 (passes 1–2 on Sept 23, pass 3 on Sept 24).** Roster moves, the group draw, the schedule and odds released after this date are not yet captured.
- Outright/match odds for TWC 2026 were **still not published** at the third-pass check (Sept 24); the betting guide deliberately contains no invented prices — only verified historical results and a framework.
...
Run `python3 scripts/verify_site.py` before publishing. It checks the eight-team/roster shape, the 60-entry ledger (three passes) and its mirror on the Verified List page, the data blocks (live Valve ranks, finals-week schedule, rulebook rules, pre-event calendar), timeline order/duplicates, retired (corrected) claims, reserve fields, and local links including #anchors; it does not replace manual review of remote sources.
